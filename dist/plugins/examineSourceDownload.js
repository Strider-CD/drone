...
function installIntoWorkflow(emitter) {
  emitter.emit('workflow.registeredEvent.install', domain, examineSourceDownloadEvent, 100, function (err, res) {
    logger.debug('installIntoWorkflow failed!', err);
  });
}

function installIntoWorkflowCleanup(emitter) {
  emitter.emit('workflow.registeredEvent.install', domain, examineSourceDownloadEvent + '.cleanup', 900, function (err, res) {
    logger.debug('installIntoWorkflowCleanup failed!', err);
  });
}
//# sourceMappingURL=examineSourceDownload.js.map